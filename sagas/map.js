...
const searchMapAPI = async (datas) => {
  try {
    const res = await fetch(
      `https://2yubi.shop/api/tracks/search?bound1=${datas.north.lng}&bound2=${datas.north.lat}&bound3=${datas.south.lng}&bound4=${datas.south.lat}&event=${datas.event}`,
      {
        method: "GET",
        headers: {
...
const createmapLoadAPI = async (datas) => {
  try {
    ...
    const res = await fetch(
      `https://2yubi.shop/api/gpsData/check?gpsId=${datas}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
...
const trackRankAPI = async (datas) => {
  const res = await axios.get(
    `https://2yubi.shop/api/ranking/track?track_id=${datas}`
  );
